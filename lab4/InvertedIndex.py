...
class InvertedIndex:
    ...
    def __init__(self, index_name, postings_encoding=None, directory=''):
        ...
        if postings_encoding is None:
            self.postings_encoding = util.UncompressedPostings
        else:
...
class InvertedIndexIterator(InvertedIndex):
    ...
    def __next__(self):
        ...
        """
        ### Begin your code
        it = self.term_iter.__next__()
        plist = self.postings_dict.get(it)       
        self.index_file.seek(plist[0])
        stream = self.index_file.read(plist[2])
        stream = self.postings_encoding.decode(stream)
        return (it, stream)
        ### End your code
...
class InvertedIndexMapper(InvertedIndex):    
    ...
    def _get_postings_list(self, term):
        ...
        """
        ### Begin your code
        if self.terms == [] or self.postings_dict == {}:
            super().__enter__()
        key = self.postings_dict.get(term)
        if key == None:
            return []
        self.index_file.seek(key[0])
        stream = self.index_file.read(key[2])
        # print(self.postings_encoding.decode(stream))
        return self.postings_encoding.decode(stream)
        ### End your code
...
class InvertedIndexWriter(InvertedIndex):
    ...
    def append(self, term, postings_list):
        ...
        """
        ### Begin your code
        stream = self.postings_encoding.encode(postings_list)
        self.terms.append(term)
        self.postings_dict.setdefault(term, (self.index_file.tell(), len(postings_list), len(stream)))
        self.index_file.write(stream)
        # print(self.terms)
